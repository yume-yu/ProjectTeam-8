...
void before_battle_st1(){
	flame_flush();
	make_flame(BATTLE_MODE_STATUS_FLAME_WIDTH,BATTLE_MODE_STATUS_FLAME_HEIGHT,BATTLE_MODE_STATUS_FLAME_X,HEIGHT - BATTLE_MODE_STATUS_FLAME_HEIGHT);
	exstrcpy(now_text,beforebattle_stage1, lengthof(beforebattle_stage1,struct extendstr));
	string_march(now_text,BATTLE_MODE_COMMAND_POS - 2,HEIGHT - BATTLE_MODE_STATUS_FLAME_HEIGHT + 1,lengthof(beforebattle_stage1,struct extendstr));
}
...
void before_battle_st2(){
	flame_flush();
	exstrcpy(now_text,beforebattle_stage2, lengthof(beforebattle_stage2,struct extendstr));
	string_march(now_text,2,2,lengthof(beforebattle_stage2,struct extendstr));
}
...
void before_battle_st3(){
	flame_flush();
	exstrcpy(now_text,beforebattle_stage3, lengthof(beforebattle_stage3,struct extendstr));
	string_march(now_text,2,2,lengthof(beforebattle_stage3,struct extendstr));
}
...
void before_battle_st4(){
	flame_flush();
	exstrcpy(now_text,beforebattle_stage4, lengthof(beforebattle_stage4,struct extendstr));
	string_march(now_text,2,2,lengthof(beforebattle_stage4,struct extendstr));
}
