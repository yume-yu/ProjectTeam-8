...
struct arrow_pos *(map_coors)[7][WIDTH - 2][HEIGHT - 2];
...
/**
 * リストを表示した際にカーソルの横縦移動と決定した項目を管理する関数
	...
	switch(now_stage){
      ...
      if(exit_point.x == 58 && exit_point.y == 18){
        ...
        start_pos.y = 17 -  2;
			...
			break;
		case stage3_2:
      if(exit_point.x == 2 && exit_point.y == 17){
        now_stage = stage3_1;
        mapcpy(now_map,maps[now_stage]);
        coorcpy(now_map_coor,map_coors[now_stage]);
        start_pos.x = 57 - 2;
        start_pos.y = 18 - 2;
			...
			break;
		case stage3_3:
      if(exit_point.x == 58 && exit_point.y == 17){
        now_stage = stage3_1;
        mapcpy(now_map,maps[now_stage]);
        coorcpy(now_map_coor,map_coors[now_stage]);
        start_pos.x = 0;
